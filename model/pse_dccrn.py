...
import torch.nn.functional as F
from .DeepComplexCRN.conv_stft import ConvSTFT, ConviSTFT 
from .DeepComplexCRN.complexnn import ComplexConv2d, ComplexConvTranspose2d, NavieComplexLSTM, complex_cat, ComplexBatchNorm
from torch.utils.checkpoint import checkpoint


class PSE_DCCRN(nn.Module):
    ...
    def __init__(
        ...
        if self.use_clstm: 
            ...
            for idx in range(rnn_layers):
                rnns.append(
                        NavieComplexLSTM(
                        input_size= (hidden_dim*self.kernel_num[-1] + 2*hp.embedder.emb_dim) if idx == 0 else self.rnn_units,
            ...
            self.enhance = nn.LSTM(
                    input_size= hidden_dim*self.kernel_num[-1] + hp.embedder.emb_dim,
                    hidden_size=self.rnn_units,
                    num_layers=2,
                    dropout=0.0,
                    bidirectional=bidirectional,
    ...
    def forward(self, inputs, dvec, lens=None):
        ...
        if self.use_clstm:
            ...
            r_rnn_in = torch.cat((r_rnn_in, dvec), dim=2)
            i_rnn_in = torch.cat((i_rnn_in, dvec), dim=2)
        ...
        for idx in range(len(self.decoder)):
            ...
            # out = checkpoint(self.decoder[idx], out)
            out = out[...,1:]
